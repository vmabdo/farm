    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden relative" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center p-6 border-b border-slate-100">
          <div>
            <h2 className="text-xl font-bold text-slate-800">Add Weight Record</h2>
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-5">
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100">
            <div>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Previous Weight</p>
              <p className="text-xl font-bold text-slate-900">{previousWeight.toFixed(2)} <span className="text-sm font-medium text-slate-500">kg</span></p>
            </div>
            {typeof newWeight === 'number' && (
              <div className="text-end">
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Variance</p>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">New Weight (kg) *</label>
            <input 
              name="weight" 
              type="number" 
              step="0.01"
          ...
          <div>
            ...
            <textarea 
              ...
              placeholder="Optional notes about this weighing..."
            ></textarea>
          </div>
