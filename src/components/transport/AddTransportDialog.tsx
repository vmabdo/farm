...
import { createTransportRent } from '@/app/actions/transport';
import { X } from 'lucide-react';

export default function AddTransportDialog({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await createTransportRent(formData);
    setLoading(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full animate-in fade-in zoom-in-95 duration-300 max-w-md overflow-hidden relative" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">تفاصيل المركبة *</label>
            <input 
              name="vehicleType" 
              required 
              className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" 
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">الغرض *</label>
            <input 
              name="purpose" 
              required 
              className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" 
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">تكلفة الرحلة (ج.م) *</label>
            <input 
              name="cost" 
              type="number" 
              min="0"
