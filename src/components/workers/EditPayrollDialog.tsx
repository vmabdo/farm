...
import { updatePayroll } from '@/app/actions/workers';
import { X } from 'lucide-react';

export default function EditPayrollDialog({ isOpen, onClose, payroll }: { isOpen: boolean; onClose: () => void; payroll: any }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await updatePayroll(payroll.id, formData);
    setLoading(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden relative" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center p-6 border-b border-slate-100">
          <h2 className="text-xl font-bold text-slate-800">Edit Salary Log</h2>
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Actual Amount Paid (EGP) *</label>
            <input 
              name="amount" 
              type="number" 
              step="0.01"
          ...
          <div>
            ...
            <input 
              name="paymentDate" 
              type="date"
              defaultValue={new Date(payroll.paymentDate).toISOString().split('T')[0]}
              required 
              className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition" 
            />
          </div>
